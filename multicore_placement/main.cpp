...
int main( int argc, const char*   argv[] )
{
    ...
    std::string out_file = "/Users/fabiocremona/Desktop/input/out_fuel_12.txt";
    std::string feta_file = "/Users/fabiocremona/Desktop/input/fecta_fuel_12.txt";
    
    Parser ps0(out_file, feta_file);
    ps0.createFromFile();
    auto fun_list = ps0.getFunctions();
    ...
    for (auto f : fun_list)
        total_load += (f->get()[0]/f->getPeriod());
    ...
    Processor p0(ps0.getFunctions(), NC);
    bool is_sched_gs = p0.interCoreAllocation(0.1);
    if (is_sched_gs == true)
    {
        std::cout << "===> Solution Schedulable <===" << std::endl;
        p0.print(std::cout);
    }
